...
const postStudentDetails = (req, res) => {
    ...
    var query = "insert into tbl_student(reg_no, std_name, dept_id, std_email, std_phone, std_address, std_dateOfBirth,password) values(?, ?, ?, ?, ?, ?, ?,'null')";
...
const postTeacherDetails = (req, res) => {
    ...
    var query = "insert into tbl_teacher(teacher_id, teacher_name, dept_id, teacher_email, teacher_phone, designation,password) values(?, ?, ?, ?, ?, ?,'null')";
...
const putAdminApproval = (req, res) => {
    ...
    console.log(req.body+'fgdfgdfgdg');
    let admin_id = req.admin_id;
    // let Exam_Controller_Approved_Date = new Date().toISOString();
    let Exam_Controller_Approved_Date = new Date().toISOString().slice(0, 19).replace('T', ' ');
